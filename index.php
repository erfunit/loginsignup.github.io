...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (is_null($cpass)) {
        $errors['cpass'] = 'Confirm password cannot be empty';
    } elseif ($cpass !== $pass) {
    ...
    if (
        ...
        $stmt = mysqli_prepare($connect, "INSERT INTO users (id, fname, lname, email, password) VALUES (null, ?, ?, ?, ?)");
    ...
    <form id="form" action="/" method="POST">
        ...
        <div class="singup">
            ...
            <?php if (has_error('pass')) { ?>
                <span><?= get_error('pass') ?></span><br>
            <?php } ?>
            <input type="password" id="cpass" placeholder="confirm password" name="cpass">
            <?php if (has_error('cpass')) { ?>
                <span><?= get_error('cpass') ?></span><br>
            <?php } ?>
        </div>
